import deleteVideoFromTrash from "../../functions/video/deleteVideoFromTrash.js";
import idVarification from "../../functions/global/token/idVarification.js";

const trashDeleteVideoController = async (req, res) => {
  ...
  try {
    const data = await deleteVideoFromTrash(id, phone, itemId);
    ...
    return res.status(200).json({
      status: true,
      data,
      message: "Permanent Delete Action on Videos",
    });
  } catch (error) {
    console.error(error);
...
export default trashDeleteVideoController;
