...
import fetchMainImage from "../../functions/photos/fetchMainImage.js";

const fetchMainImageCobtroller = async (req, res) => {
  ...
  try {
    const photos = await fetchMainImage(req.body.id);
    if (photos.error) {
      return res.status(404).json({
        message: check.error.message,
      });
...
export default fetchMainImageCobtroller;
